...
class mav_viewer():
    ...
    def _get_spacecraft_points(self):
        ...
        meshColors = np.empty((16, 3, 4), dtype=np.float32)
        meshColors[0] = red  # front
        meshColors[1] = red  # front
        meshColors[2] = red  # back
        meshColors[3] = red  # back
        meshColors[4] = blue  # right
        meshColors[5] = blue  # right
        meshColors[6] = blue  # left
        meshColors[7] = blue  # left
        meshColors[8] = yellow  # top
        meshColors[9] = green  # top
        meshColors[10] = red  # bottom
        meshColors[11] = red  # bottom
        meshColors[12] = blue  # top
        meshColors[13] = blue  # top
        meshColors[14] = green  # bottom
        meshColors[15] = green  # bottom
    ...
    def _points_to_mesh(self, points):
        ...
        """
        points=points.T
        mesh = np.array([[points[0], points[1], points[2]],  # head
                         [points[3], points[2], points[0]],  # head
                         [points[0], points[4], points[3]],  # head
                         [points[1], points[0], points[4]],  # head
                         [points[1], points[5], points[2]],  # body
                         [points[5], points[3], points[2]],  # body
                         [points[5], points[3], points[4]],  # body
                         [points[5], points[4], points[1]],  # body
                         [points[5], points[14], points[15]],  # verticla rear  
                         [points[1], points[2], points[3]],  # head inside
                         [points[1], points[3], points[4]],  # head inside
                         [points[8], points[7], points[6]],  # wing1
                         [points[9], points[6], points[8]],  # wing2
                         [points[10], points[11], points[12]],  # horizon rear
                         [points[10], points[12], points[13]],  # horizon rear
                         ])
