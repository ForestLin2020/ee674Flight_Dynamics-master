...
class ekf_attitude:
    ...
    def propagate_model(self, state):
        ...
        for i in range(0, self.N):
            ...
            # compute Jacobian
            A = jacobian(self.f, self.xhat,
                         state)  # missing brackets on the columns-> np.array([[q * np.cos(phi) * np.tan(theta) - r * np.sin(phi) * np.tan(theta), (q*np.sin(phi)-r*np.cos(phi))/np.cos(theta)**2.], [-q * np.sin(phi) - r * np.cos(theta), 0]])
