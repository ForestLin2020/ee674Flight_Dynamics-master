...
class mav_dynamics:
    ...
    def update_sensors(self):
        ...
        self.sensors.gyro_x = self._state.item(10) + np.random.normal(SENSOR.gyro_x_bias, SENSOR.gyro_sigma)
        self.sensors.gyro_y = self._state.item(11) + np.random.normal(SENSOR.gyro_y_bias, SENSOR.gyro_sigma)
        self.sensors.gyro_z = self._state.item(12) + np.random.normal(SENSOR.gyro_z_bias, SENSOR.gyro_sigma)
        self.sensors.accel_x = np.asscalar((self._forces.item(0) / MAV.mass) + MAV.gravity * sin(theta) + np.random.normal(0, SENSOR.accel_sigma))
        self.sensors.accel_y = np.asscalar((self._forces.item(1) / MAV.mass) - MAV.gravity * cos(theta) * sin(phi) + np.random.normal(0, SENSOR.accel_sigma))
        self.sensors.accel_z = np.asscalar((self._forces.item(2) / MAV.mass) - MAV.gravity * cos(theta) * cos(phi) + np.random.normal(0, SENSOR.accel_sigma))
        self.sensors.static_pressure = MAV.rho * MAV.gravity * self._state.item(2) + np.random.normal(0, SENSOR.static_pres_sigma)
        self.sensors.diff_pressure = (MAV.rho * self._Va**2)/2 + np.random.normal(0, SENSOR.diff_pres_sigma)
        if self._t_gps >= SENSOR.ts_gps:
            ...
            self.sensors.gps_Vg = np.asscalar(np.sqrt((self._Va*cos(psi)+self.msg_true_state.wn)**2 + (self._Va*sin(psi)+self.msg_true_state.we)**2) + np.random.normal(0,SENSOR.gps_Vg_sigma))
            self.sensors.gps_course = np.asscalar(np.arctan2(self._Va*sin(psi)+self.msg_true_state.we, self._Va*cos(psi)+self.msg_true_state.wn) + np.random.normal(0,SENSOR.gps_Vg_sigma))
            self._t_gps = 0.
        else:
    ...
    def _forces_moments(self, delta):
        ...
        fx = (-mass * gravity * sin(theta)) + Tp + (1 / 2) * rho * Va ** 2 * S * (
                    C_X_alpha + (C_X_q_alpha * MAV.c * q) / (2 * Va)) + (1 / 2) * rho * Va ** 2 * S * (
                         C_X_delta_e_alpha * delta[0])
        fy = (mass * gravity * cos(theta) * sin(phi)) + (1 / 2) * rho * Va ** 2 * S * (
                    C_Y_0 + C_Y_beta * beta + (C_Y_p * MAV.b * p) / (2 * Va) + (C_Y_r * MAV.b * r) / (2 * Va)) + (
                         1 / 2) * rho * Va ** 2 * S * (C_Y_delta_a * delta[2] + C_Y_delta_r * delta[3])
        fz = (mass * gravity * cos(theta) * cos(phi)) + (1 / 2) * rho * Va ** 2 * S * (
                    C_Z_alpha + (C_Z_q_alpha * MAV.c * q) / (2 * Va)) + (1 / 2) * rho * Va ** 2 * S * (
                         C_Z_delta_e_alpha * delta[0])
        ...
        l = (1 / 2) * rho * Va ** 2 * S * (MAV.b * (C_ell_0 + C_ell_beta * beta + ((C_ell_p * MAV.b * p) / (2 * Va)) + (
                    (C_ell_r * MAV.b * r) / (2 * Va)))) + (1 / 2) * rho * Va ** 2 * S * (
                        MAV.b * (C_ell_delta_a * delta[2] + C_ell_delta_r * delta[3]))
        m = (1 / 2) * rho * Va ** 2 * S * (
                    MAV.c * (MAV.C_m_0 + MAV.C_m_alpha * alpha + (MAV.C_m_q * MAV.c * q) / (2 * Va))) + (
                        1 / 2) * rho * Va ** 2 * S * (MAV.c * (MAV.C_m_delta_e * delta[0]))
        n = (1 / 2) * rho * Va ** 2 * S * (MAV.b * (
                    C_n_0 + C_n_beta * beta + (C_n_p * MAV.b * p) / (2 * Va) + (C_n_r * MAV.b * r) / (2 * Va))) + (
                        1 / 2) * rho * Va ** 2 * S * (MAV.b * (C_n_delta_a * delta[2] + C_n_delta_r * delta[3]))
    ...
    def _update_msg_true_state(self):
        ...
        self.msg_true_state.gamma = np.arcsin(
            np.sqrt(pn_dot ** 2 + pe_dot ** 2) / np.sqrt(pn_dot ** 2 + pe_dot ** 2 + pd_dot ** 2))
